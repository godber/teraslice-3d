...
- **Physics / Layout**: Swift-based 3D Force Layout simulation (or pre-computed 3D positions from layout engine)
...
| :--- | :--- | :--- |
| **3D Rendering** | Three.js / `3d-force-graph` with sphere nodes & tube links | **RealityKit `ModelEntity`**: Spheres for nodes, cylinders/pipe meshes for job links. Dynamic thickness based on worker counts. |
...
| **Auto-Refresh** | Polling timer (10s–300s) with live update check & status badge | **Swift `Task` / Timer Publisher**: Auto-updates RealityKit entity positions/materials with connection status indicator in UI ornament. |
...
│   └── ForceSimulation3D.swift     # 3D layout simulation engine
├── RealityKit/
│   ├── PipelineGraphEntity.swift   # Root RealityKit Entity
...
- [ ] **Chunk 2: 3D Force-Directed Layout Engine & Physics Simulation** *(Code Written, Unit Tests Passing — Pending Visual Verification)*
- [ ] **Chunk 3: RealityKit 3D Renderer & Volumetric Window Container** *(In Progress — Baseline Single Ball Verified, Graph Rendering Debugging)*
...
### **Chunk 2: 3D Force Simulation & Layout Engine**
- **Status**: 🔄 Code Implemented (Pending Visual Verification)
- **Goal**: Build pure Swift 3D force-directed layout engine and unit test suite.
- **Tasks & Deliverables**:
  1. [x] **3D Force Engine (`Services/ForceSimulation3D.swift`)**: Coulomb node repulsion, Hooke link attraction, velocity damping, gravity, and bounding box constraints.
  2. [x] **Unit Tests (`Tests/`)**: Unit test suite passing 5/5 tests (`testForceSimulationSetupAndTicks`, `testForceSimulationBounds`, etc.).
  3. [ ] **Visual 3D Verification**: Pending visual verification in visionOS simulator once RealityKit rendering pipeline is fixed.

### **Chunk 3: RealityKit Renderer & Volumetric Window Container**
- **Status**: 🔄 In Progress / Troubleshooting
- **Goal**: Render interactive 3D nodes and links inside visionOS RealityKit scene inside a volumetric window.
- **Tasks & Deliverables**:
  ...
  2. [ ] **Node Entities (`RealityKit/NodeEntity.swift`)**: Sphere entities with color-coded materials (Kafka Yellow, ES Green, etc.) — *Troubleshooting visual placement*.
  3. [ ] **Link Entities (`RealityKit/LinkEntity.swift`)**: Dynamic pipe meshes with worker count thickness — *Troubleshooting geometry warping/clipping*.
  4. [ ] **Root Graph Entity (`RealityKit/PipelineGraphEntity.swift`)**: Parent entity managing layout updates — *Troubleshooting scene graph rendering*.
...
- **Tasks & Deliverables**:
  1. [x] **Control Ornament (`Views/ControlOrnamentView.swift`)**: Bottom ornament attached to volumetric window with search bar, filter toggle, refresh trigger, server selector, inspector toggle, and settings sheet.
  2. [x] **Jobs Inspector Drawer (`Views/JobsInspectorView.swift`)**: Searchable, sortable list view displaying live backend jobs.
...
- **3D RealityKit Baseline**: `PipelineVolumeView.swift` currently renders a single green sphere at `(0, 0, 0)` in `RealityView`, verified working in visionOS Simulator.

### 🎯 Immediate Next Steps for Next Session
1. **Incremental 3D Graph Rendering (Step 3A)**:
   - Render 2 static node spheres at fixed coordinates (e.g. `(-0.15, 0, 0)` and `(0.15, 0, 0)`) directly inside `RealityView { content in ... }`.
   - Verify visually in visionOS Simulator that multiple spheres render at distinct 3D positions.
2. **Enable Force Simulation & Pipe Links (Step 3B & 3C)**:
   - Wire `ForceSimulation3D` node positions to the rendered spheres inside `RealityView`.
   - Connect node spheres with 3D pipe meshes (`LinkEntity.swift`) using safe quaternion orientation.
