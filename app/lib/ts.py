def process_source_node(job, logger):
    op = job['operations'][0]
...
def process_destination_node(job, logger):
    op = job['operations'][-1]
...
def process_job(job, logger):
    ...
    * destination_type
    ...
    source_node, source_type = process_source_node(job, logger)
    destination_nodes, destination_type = process_destination_node(job, logger)
